...
class 刷题应用(tk.Tk):
    ...
    def 显示识别性能报告(self):
        ...
        try:
            # 使用原始识别系统
            from 智能题型识别 import get_recognition_performance
            
            报告 = get_recognition_performance()
            
            报告文本 = f"""
🧠 智能题型识别系统 - 性能报告

📊 总体统计:
• 总调用次数: {报告['total_calls']}
• 总成功次数: {报告['total_success']}
• 识别历史记录: {报告['recognition_history_count']}

🔧 各模式性能:
"""
            
            # 添加各模式性能统计
            for mode, stats in 报告['performance_stats'].items():
                if stats['调用次数'] > 0:
                    success_rate = stats['成功次数'] / stats['调用次数']
                    报告文本 += f"• {mode}模式: 成功率 {success_rate:.1%}, 平均耗时 {stats['平均耗时']:.4f}s\n"
...
"""
            ...
            messagebox.showerror("错误", f"获取统一识别系统性能报告失败：\n{e}")
